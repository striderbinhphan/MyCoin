...
class Wallets{
    ...
    //publicKey == address
    validKey(inputPublicKey, inputPrivateKey){
        const key = ec.keyFromPublic(inputPublicKey,'hex');
        const privateKey = key.getPrivate('hex');
        if(inputPrivateKey === privateKey){
            return true;
        }
        return false;
    }
    addAddress(inputPublicKey){
        if(!this.walletArray.includes(inputPublicKey)){
            this.walletArray.push(inputPublicKey)
        }else{
